...
The key difference is the `family` argument, which describes the error distribution and specify a link function.
...
```{r}
# using dcast for the totals
# this is an example of something dcast can do but tidyr::spread can't
# obesity defines rows  ~  demog becomes multiple columns
obese %>% dcast(obese ~ demog, fun.aggregate = length) 
# but to get the proportions, you'd probably convert
# to a matrix and use prop.table()
```
