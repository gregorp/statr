...
R everything we do tonight can be done many different ways, in base R or using any number of packages. Instead of focusing on that, I'm highlighting a small number of relatively new, well-maintained packages that have helped me get these jobs done easily and efficiently.
...
TODO link to Tidy Data paper

Data that follows Tidy Data principles is easy to manipulate and query, easy to 
plot (with ggplot, at least), and easy to use in a model.

---

```{r, eval = FALSE}
# TODO eval truee
fortunes::fortune("tidy data")
```
...
re2 = melt(re2, id.vars = "X", variable.name = "months", value.name = "percent")
...
The above uses are illustrative, but pointless in practice.
...
TODO picture of model with broom?
